...
-- Fix wallet balance mismatch check to use correct column name
...
BEGIN
  -- 1. Offers stuck in 'accepted' for >14 days
  ...
  SELECT 'milestone', m.id, 'stale_milestone',
    'Milestone submitted but not reviewed, past auto-release deadline', 'high'
  FROM milestones m
  ...
  SELECT 'wallet', w.id, 'orphaned_escrow',
    'Wallet has escrow balance ' || w.escrow_balance || ' but no active offers', 'critical',
    jsonb_build_object('escrow_balance', w.escrow_balance, 'user_id', w.user_id)
  FROM wallets w
  WHERE w.escrow_balance > 0
    AND NOT EXISTS (SELECT 1 FROM offers o WHERE (o.sender_id = w.user_id OR o.recipient_id = w.user_id) AND o.status IN ('accepted', 'in_progress'))
  ...
  -- 4. Wallet balance mismatches (available + escrow + pending should be reasonable)
  SELECT COUNT(*) INTO v_wallet_mismatches
  FROM wallets w
  WHERE (w.available_balance + w.escrow_balance + COALESCE(w.pending_balance, 0)) < 0;
  ...
  IF v_orphaned_escrows > 0 OR v_wallet_mismatches > 0 THEN v_health := 'critical';
  ELSIF v_stuck_offers > 3 OR v_stuck_milestones > 3 THEN v_health := 'degraded';
  END IF;

  INSERT INTO execution_health_snapshots (stuck_offers, stuck_milestones, orphaned_escrows, wallet_mismatches, stale_processing_keys, overall_health, details)
  VALUES (v_stuck_offers, v_stuck_milestones, v_orphaned_escrows, v_wallet_mismatches, v_stale_keys, v_health,
    jsonb_build_object('stuck_offers', v_stuck_offers, 'stuck_milestones', v_stuck_milestones, 'orphaned_escrows', v_orphaned_escrows, 'wallet_mismatches', v_wallet_mismatches, 'stale_keys', v_stale_keys));

  RETURN jsonb_build_object('health', v_health, 'stuck_offers', v_stuck_offers, 'stuck_milestones', v_stuck_milestones,
    'orphaned_escrows', v_orphaned_escrows, 'wallet_mismatches', v_wallet_mismatches, 'stale_processing_keys', v_stale_keys);
END;
$$;
