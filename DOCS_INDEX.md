# Docs Index

This file lists the main ResearchCollab project documents.

## Start here

1. `README.md`
2. `SUPPORT.md`
3. `SUPPORT_NOTES_TEMPLATE.md`
4. `PROGRESS_150.md`
5. `PROGRESS_170.md`
6. `PROGRESS_180.md`
7. `PROGRESS_190.md`
8. `DOC_NOTE_195.md`
9. `DOC_NOTE_200.md`
10. `DOC_NOTE_218.md`
11. `CHANGELOG.md`
12. `CONTRIBUTING.md`
13. `RELEASE_CHECKLIST.md`
14. `RELEASE_NOTES_TEMPLATE.md`
15. `BUILD_VERIFICATION.md`
16. `MANUAL_SMOKE_TEST.md`
17. `SMOKE_TEST_NOTES_TEMPLATE.md`
18. `DEPLOYMENT_GUIDE.md`
19. `SEO_DOMAIN_AUDIT.md`
20. `OPERATIONS_RUNBOOK.md`
21. `SECURITY.md`

## Project

- `README.md` — project overview
- `SUPPORT.md` — guide
- `SUPPORT_NOTES_TEMPLATE.md` — note format
- `PROGRESS_150.md` — progress note
- `PROGRESS_170.md` — progress note
- `PROGRESS_180.md` — progress note
- `PROGRESS_190.md` — progress note
- `DOC_NOTE_195.md` — doc note
- `DOC_NOTE_200.md` — doc note
- `DOC_NOTE_218.md` — doc note
- `CHANGELOG.md` — change history
- `CONTRIBUTING.md` — contribution guide
- `.env.example` — environment example

## Release

- `RELEASE_CHECKLIST.md` — checklist
- `RELEASE_NOTES_TEMPLATE.md` — notes template
- `.github/pull_request_template.md` — PR checklist
- `.github/CODEOWNERS` — owners

## Deployment

- `DEPLOYMENT_GUIDE.md` — deployment guide
- `OPERATIONS_RUNBOOK.md` — operations checklist

## SEO and domain

- `SEO_DOMAIN_AUDIT.md` — SEO and domain checklist
- `.github/ISSUE_TEMPLATE/seo_domain_audit.yml` — SEO/domain issue template

## Build and security

- `BUILD_VERIFICATION.md` — build guide
- `MANUAL_SMOKE_TEST.md` — manual test guide
- `SMOKE_TEST_NOTES_TEMPLATE.md` — test notes template
- `.github/workflows/build-check.yml` — build workflow
- `.github/ISSUE_TEMPLATE/build_lint_failure.yml` — build issue template
- `.github/ISSUE_TEMPLATE/manual_smoke_test.yml` — manual test template
- `.github/ISSUE_TEMPLATE/support_note.yml` — support note template
- `SECURITY.md` — policy
